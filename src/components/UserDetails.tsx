      ...
      <div className="card bg-base-100 w-100 shadow-xl">
        ...
        <div className="card-body items-center text-center">
          <label className="input input-bordered flex items-center gap-2">
            <input type="text" className="grow" placeholder={user?.name} onChange={
              (e)=>{
          ...
          <label className="input input-bordered flex items-center gap-2">
            <input type="text" className="grow" placeholder="Profile Pic." 
            onChange={
              (e)=>{
            ...
            <button className="btn btn-primary"
              onClick={()=>{
                const ud: UserType = {
                  name: name,
                  email: user?.email || '',
                  profilePic: profilePic || 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRWKcuEQaieI_OrYZoI5b8u2vxu9KZkthFPdA&s',
                ...
                .then((res:any)=>{
                  if(res.isUpdated){
                    showAlert(alertTypes.SUCCESS,"User Updated Successfully","🎊")
                  }else{
                    showAlert(alertTypes.ERROR,"User not Updated","🥴")
                  }
              
                }).catch((error:any)=>{
                  showAlert(alertTypes.ERROR,"An error occurred while updating user "+error,"😢")
                  console.log(error)
